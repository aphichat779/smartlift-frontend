...
export const tasksService = {
  ...
  technicianProfile(id) {
    return apiService.get(
      `/api/work/techprofile.php?id=${encodeURIComponent(id)}&with_stats=1`
    );
  },
  ...
  reports(params = {}) {
    ...
    return apiService.request(
      `/api/admin/tasks/reports_open.php${qs ? `?${qs}` : ""}`
    );
  },
  ...
  reportProgressByRpId(rp_id) {
    return apiService.request(
      `/api/work/reports.php?progress=1&rp_id=${encodeURIComponent(rp_id)}`
    );
  },
  reportProgressByTkId(tk_id) {
    return apiService.request(
      `/api/work/reports.php?progress=1&tk_id=${encodeURIComponent(tk_id)}`
    );
  },
};
