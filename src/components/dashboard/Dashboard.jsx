...
function IconBox({ icon: Icon = Activity, tone = "emerald" }) {
  const map = {
  ...
  return (
    <div className={cx("h-10 w-10 grid place-items-center rounded-xl border", map[tone] || map.slate)}>
...
function StatusPill({ text }) {
  ...
  return <span className={cx("px-2 py-1 rounded-md text-xs", map[variant])}>{text || "—"}</span>;
}
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-24 rounded-xl bg-slate-200/70 animate-pulse" />
          ))}
        </div>
...
function AdminPanel({ data }) {
  const rows = data?.charts?.liftsByOrg || [];
  return (
    <Section title="ลิฟต์ตามองค์กร | Lifts by Org">
      {rows.length ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Card>
            <CardHeader><CardTitle>ตาราง</CardTitle></CardHeader>
          ...
          <Card>
            <CardHeader><CardTitle>แผนภูมิ</CardTitle></CardHeader>
            <CardContent className="pt-6 h-72">
              {rows.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={rows} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                    <XAxis dataKey="org_name" tick={{ fontSize: 12 }} interval={0} angle={-10} height={40} />
                    <YAxis width={32} />
                    <Tooltip />
                    <Bar dataKey="lifts" radius={[6,6,0,0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <EmptyState desc="ไม่มีข้อมูลสำหรับวาดกราฟ" />
              )}
            </CardContent>
          </Card>
        </div>
      ) : (
        <Card><CardContent><EmptyState title="ยังไม่มีข้อมูลองค์กร" /></CardContent></Card>
      )}
    </Section>
  );
}

function OrgAdminPanel({ data }) {
  const rows = data?.charts?.liftsByBuilding || [];
  return (
    <Section title="ลิฟต์ตามอาคาร | Lifts by Building">
      {rows.length ? (
          ...
          <Card>
            ...
            <CardContent className="pt-6 h-72">
              {rows.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={rows} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
                    <XAxis dataKey="building_name" tick={{ fontSize: 12 }} interval={0} angle={-10} height={40} />
                    <YAxis width={32} />
                    <Tooltip />
                    <Bar dataKey="lifts" radius={[6,6,0,0]} />
                  </BarChart>
        ...
        <Card><CardContent><EmptyState title="ยังไม่มีข้อมูลอาคาร" /></CardContent></Card>
      )}

      {/* Recent tasks (if provided) */}
      {Array.isArray(data?.tables?.recentTasks) && (
        <Section title="งานล่าสุด | Recent Tasks" className="mt-6">
          {data.tables.recentTasks.length ? (
            ...
            <Card><CardContent><EmptyState title="ยังไม่มีงานล่าสุด" /></CardContent></Card>
          )}
        </Section>
      )}
    </Section>
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mb-8">
      <Card>
        ...
        <CardContent>
          {Array.isArray(data?.tables?.myQueue) && data.tables.myQueue.length ? (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>#</TableHead>
      ...
      <Card>
        ...
        <CardContent>
          {Array.isArray(data?.tables?.recentLiftLogs) && data.tables.recentLiftLogs.length ? (
...
function UserPanel({ data }) {
  const lifts = data?.tables?.myLifts || [];
  return (
              ...
              {lifts.map((r) => (
                <li key={r.id} className="rounded-xl border p-3 flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <div className="text-muted-foreground text-xs">{r.building_name ?? "—"}</div>
                  </div>
                  <Badge variant="outline" className="whitespace-nowrap">max {numberFmt(r.max_level)}</Badge>
                </li>
              ))}
            </ul>
          </CardContent>
...
export default function Dashboard() {
  ...
  const effectiveRole = useMemo(() => data?.role || role, [data?.role, role]);
  ...
  const overviewCards = Array.isArray(data?.cards) ? data.cards : [];
  ...
  return (
    <div className="w-full pl-4 pr-4 md:pl-6 md:pr-6 lg:pl-8 lg:pr-8">
        ...
        <div className="relative overflow-hidden rounded-2xl border bg-gradient-to-r from-emerald-50 via-sky-50 to-violet-50">
          <div className="p-5 md:p-6 lg:p-8">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl md:text-2xl font-extrabold text-gray-900">ยินดีต้อนรับ, {user?.first_name}!</h2>
      ...
      {!isLoading && !error && (
        <Section title="สถานะบัญชี | Account">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-6 mb-2">
            <Metric title="สถานะบัญชี" value={<span className="text-emerald-700">ใช้งานได้</span>} icon={Activity} tone="emerald" />
            <Metric
            ...
            {user?.ga_enabled ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Card className="flex flex-col justify-center items-center p-6 border-dashed">
                  ...
                  <TotpConfirmButton onConfirmSuccess={handleTotpConfirmed} buttonText="ตรวจสอบ" buttonVariant="destructive" />
                </Card>
              </motion.div>
            ) : (
      ...
      {!isLoading && !error && overviewCards.length > 0 && (
        <Section title="สรุปภาพรวม | Overview">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-6 mb-2">
            {overviewCards.map((c, idx) => {
              const label = c.label || "";
              const value = numberFmt(c.value);
              const icon = label.includes("Orgs") ? Server : label.includes("Buildings") ? Building2 : (label.toLowerCase().includes("users") || label.includes("ผู้ใช้")) ? Users : (label.toLowerCase().includes("lifts") || label.includes("ลิฟต์")) ? Home : Activity;
              const tone = label.includes("Orgs") ? "violet" : label.includes("Buildings") ? "sky" : (label.toLowerCase().includes("users") || label.includes("ผู้ใช้")) ? "amber" : "emerald";
              return <Metric key={idx} title={label} value={value} icon={icon} tone={tone} />;
            })}
          </div>
        </Section>
      )}
      ...
      {!isLoading && !error && (
        <AnimatePresence mode="wait">
          {effectiveRole === "admin" && <AdminPanel key="admin" data={data} />}
          {effectiveRole === "org_admin" && <OrgAdminPanel key="org_admin" data={data} />}
          {effectiveRole === "technician" && <TechnicianPanel key="technician" data={data} />}
          {effectiveRole === "user" && <UserPanel key="user" data={data} />}
        </AnimatePresence>
      )}
    </div>
  );
